...
window.Examen = (function () {
  ...
  function renderAnalisis() {
    ...
    const intento = {
      ...
      respuestasModelo: [
        t.respuesta_modelo_a || t.respuesta_modelo || '',
        t.respuesta_modelo_b || t.respuesta_modelo || '',
        t.respuesta_modelo_c || t.respuesta_modelo || ''
      ],
      score: a.score,
      longitud: a.longitud,
      ...
      <div class="examen-pantalla">
        <div class="examen-card examen-card-result">
          ...
          <div class="analisis-detalle">
            <h4>📝 Tus respuestas y modelos</h4>
            ${[ps.a, ps.b, ps.c].map((q, i) => {
              const modelo = t['respuesta_modelo_' + ['a','b','c'][i]] || t.respuesta_modelo || '(sin modelo)';
              return `
              <details ${i === 0 ? 'open' : ''} style="margin:0.5rem 0">
                <summary><strong>Q${i+1}:</strong> ${escapeHtml(q)}</summary>
                <p class="hist-line"><strong>👤 Tu respuesta:</strong></p>
                <p class="hist-resp">${escapeHtml(state.respuestas[i] || '(sin respuesta)')}</p>
                <p class="hist-line"><strong>🎯 Modelo:</strong></p>
                <p class="comparar-texto">${escapeHtml(modelo)}</p>
              </details>
            `;}).join('')}
          </div>
  ...
  function copiarPromptClaude(intento) {
    ...
    const modelos = intento.respuestasModelo || ['', '', ''];
    const prompt =
`Soy un estudiante preparando el Aptis ESOL General Speaking (apunto a C1).
Acabo de hacer la Parte ${partNum} (tema: ${intento.tema}).

PREGUNTAS, MIS RESPUESTAS Y MODELOS DE REFERENCIA:

Q1: ${intento.preguntas.a}
MI RESPUESTA: ${intento.respuestas[0] || '(vacía)'}
MODELO: ${modelos[0] || '(sin modelo)'}

Q2: ${intento.preguntas.b}
MI RESPUESTA: ${intento.respuestas[1] || '(vacía)'}
MODELO: ${modelos[1] || '(sin modelo)'}

Q3: ${intento.preguntas.c}
MI RESPUESTA: ${intento.respuestas[2] || '(vacía)'}
MODELO: ${modelos[2] || '(sin modelo)'}
...
4. Compara mis respuestas con los modelos: qué me falta y qué hago bien.`;
    try {
  ...
  function guardarSimulacroParte() {
    state.simulacroDatos.partes.push({
      ...
      respuestas: state.respuestas.slice(),
      respuestasModelo: [
        state.tema.respuesta_modelo_a || state.tema.respuesta_modelo || '',
        state.tema.respuesta_modelo_b || state.tema.respuesta_modelo || '',
        state.tema.respuesta_modelo_c || state.tema.respuesta_modelo || ''
      ]
    });
  }
    ...
    host.innerHTML = `
      <div class="examen-pantalla">
        <div class="examen-card examen-card-result" style="border-top-color:var(--bad)">
          ...
          <div class="analisis-detalle">
            <h4>📋 Resumen por parte (con modelos)</h4>
            ${sd.partes.map((part, i) => `
              <details ${i === 0 ? 'open' : ''} style="margin:0.5rem 0">
                ...
                ${part.respuestas.map((r, j) => `
                  ...
                  <p class="hist-line"><strong>🎯 Modelo:</strong></p>
                  <p class="comparar-texto">${escapeHtml((part.respuestasModelo && part.respuestasModelo[j]) || '(sin modelo)')}</p>
                `).join('')}
              </details>
            `).join('')}
          </div>
...
`;
    sd.partes.forEach((part, i) => {
      ...
      const modelos = part.respuestasModelo || ['', '', ''];
      [part.preguntas.a, part.preguntas.b, part.preguntas.c].forEach((q, j) => {
        prompt += `\nQ${j+1}: ${q}\nMI RESPUESTA: ${part.respuestas[j] || '(vacía)'}\nMODELO: ${modelos[j] || '(sin modelo)'}\n`;
      });
    });
    prompt += `\n\nEvalúame como examinador Aptis:
...
5. Compara mis respuestas con los modelos: qué me falta de cada uno.
    ...
    try {
      ...
      sd.partes.forEach((part, i) => {
        ...
        [part.preguntas.a, part.preguntas.b, part.preguntas.c].forEach((q, j) => {
          ...
          doc.setFont('helvetica', 'bold');
          doc.setFontSize(9);
          doc.text('Tu respuesta:', marginX + 5, y); y += 4;
          doc.setFont('helvetica', 'normal');
          ...
          if (modelos[j]) {
            ...
            doc.setFontSize(9);
            doc.setTextColor(15, 23, 42);
            doc.text('Modelo:', marginX + 5, y); y += 4;
            doc.setFont('helvetica', 'normal');
            doc.setTextColor(60);
            const ml = doc.splitTextToSize(modelos[j], pageW - marginX*2 - 5);
            if (y + ml.length * 4.5 > 280) { doc.addPage(); y = 20; }
            doc.text(ml, marginX + 5, y);
            y += ml.length * 4.5 + 2;
            doc.setTextColor(0);
          }
          if (j < 2) y += 2;
...
5. Plan de mejora semanal.`;
  ...
  function pintarSeccionIntento(doc, intento, y, marginX, pageW) {
    ...
    [ps.a, ps.b, ps.c].forEach((q, i) => {
      ...
      if (y + rl.length * 4.5 > 280) { doc.addPage(); y = 20; }
      doc.text(rl, marginX + 5, y); y += rl.length * 4.5 + 2;
      // Modelo
      if (modelos[i]) {
        doc.setFont('helvetica', 'bold');
        doc.setFontSize(9);
        ...
        doc.setFont('helvetica', 'normal');
        doc.setTextColor(60);
        const ml = doc.splitTextToSize(modelos[i], pageW - marginX*2 - 5);
        if (y + ml.length * 4.5 > 280) { doc.addPage(); y = 20; }
        doc.text(ml, marginX + 5, y); y += ml.length * 4.5 + 2;
        doc.setTextColor(0);
      }
      // Separador
      if (i < 2) { y += 2; }
