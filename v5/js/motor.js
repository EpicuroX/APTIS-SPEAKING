...
(function () {
    ...
    switch (state.mode) {
      ...
      case 'simulacro':
        if (window.Examen) {
          main.innerHTML = head + '<div id="examen-host"></div>';
          window.Examen.mount(document.getElementById('examen-host'), state.part, p);
          window.Examen.iniciarSimulacro();
          return;
        }
        body = '<p class="error">Simulacro no disponible.</p>';
        break;
    }
  ...
  function afterRender() {
    document.querySelectorAll('.tpl-hueco').forEach(btn => {
      btn.addEventListener('click', (e) => {
        ...
        openPopover(btn.dataset.part, btn.dataset.paso, btn.dataset.hueco, btn);
      });
    });
    document.querySelectorAll('.step-connector-more').forEach(btn => {
      btn.addEventListener('click', () => openConectorPopover(btn.dataset.part, btn.dataset.paso));
    });
    ...
    document.querySelectorAll('.conector-item').forEach(li => {
      li.addEventListener('click', () => {
        ...
        // Si el texto incluye traducción ('inglés → español'), leer solo el inglés.
        const soloIngles = t.split('→')[0].trim();
        speak(soloIngles || t);
      });
    });
  ...
  function renderEstructura(p) {
    ...
    // Si la parte tiene `plantillas` (múltiples) en vez de `pasos` (única), renderizamos las 3 plantillas
    if (p.plantillas && p.plantillas.length) {
      if (p.patron_oficial) {
        html += `
          <div class="conector-grupo" style="border-left-color:var(--accent)">
            <div class="conector-grupo-head"><span class="conector-grupo-icon">📖</span><h3 class="conector-grupo-label">${escapeHtml(p.patron_oficial.titulo)}</h3></div>
            <p class="conector-cuando">${escapeHtml(p.patron_oficial.descripcion)}</p>
            ${p.patron_oficial.ejemplo_real.map(e => `<p class="conector-ejemplo">${escapeHtml(e)}</p>`).join('')}
          </div>
        `;
      }
      p.plantillas.forEach(pl => {
        html += `
          <div class="step-card" style="border-top-color:${pl.color}">
            <div class="step-head">
              <span class="step-num" style="background:${pl.color}">${escapeHtml(pl.tipo)}</span>
              <div class="step-titles">
                <h3 class="step-title">${escapeHtml(pl.label)}</h3>
                <p class="step-time">${escapeHtml(pl.objetivo_palabras)} · ${escapeHtml(pl.uso)}</p>
              </div>
            </div>
          </div>
        `;
        pl.pasos.forEach(paso => { html += renderPaso(paso, p.id, pl.tipo); });
      });
    } else {
  ...
  function renderPaso(paso, partId, plantillaTipo) {
    const pasoIdComp = plantillaTipo ? `${plantillaTipo}-${paso.n}` : `${paso.n}`;
      ...
      ? `
        <div class="step-connector">
          ...
          ${paso.conector_inicial.alternativas && paso.conector_inicial.alternativas.length
            ? `<button class="step-connector-more" data-part="${partId}" data-paso="${pasoIdComp}">+ alt</button>`
            : ''}
        </div>
      `
    ...
    const plantillaHtml = paso.plantilla_segmentos
      ? renderSegmentos(paso.plantilla_segmentos, paso.huecos || {}, partId, pasoIdComp)
      : '<em>(sin plantilla)</em>';
  ...
  // ============== POPOVER ==============
  function findPaso(p, pasoId) {
    // pasoId puede ser "3" (única plantilla) o "A-2" (plantilla A, paso 2)
    const s = String(pasoId);
    if (s.indexOf('-') >= 0) {
      const [tipo, n] = s.split('-');
      if (p.plantillas) {
        const pl = p.plantillas.find(x => x.tipo === tipo);
        if (pl) return pl.pasos.find(x => x.n === parseInt(n, 10));
      }
      return null;
    }
    const n = parseInt(s, 10);
    if (p.pasos) return p.pasos.find(x => x.n === n);
    if (p.plantillas) {
      // buscar en todas las plantillas
      for (const pl of p.plantillas) {
        const found = pl.pasos.find(x => x.n === n);
